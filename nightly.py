...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help=_("Do not resign the apk's. Eg. a system app that depends on the system key."),
    )
    parser.add_argument(
        "--file",
            ...
            for f in files:
                if f.endswith('-debug.apk'):
                    ...
                    if not options.no_resign:
                        ...
                        logging.debug(
                            _(
                                'Not resigning {apkfilename}: moving file.'
                            ).format(apkfilename=os.path.basename(apkfilename))
                        )
                        shutil.move(apkfilename, destapk)
